...
class TestLc0241DiffWaysToCompute:
    ...
    def test_basic(self):
        result = self.sol.diffWaysToCompute("2-1-1")
        assert sorted(result) == sorted([0, 2])

    def test_multiply(self):
        result = self.sol.diffWaysToCompute("2*3-4*5")
        assert sorted(result) == sorted([-34, -14, -10, -10, 10])

    def test_single_number(self):
        assert self.sol.diffWaysToCompute("3") == [3]

    def test_simple_add(self):
        assert self.sol.diffWaysToCompute("1+1") == [2]
